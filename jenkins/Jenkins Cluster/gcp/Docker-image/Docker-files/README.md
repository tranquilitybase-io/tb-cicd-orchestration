# Building and Jenkins Docker image

Make changes to the jenkins configuration files as required and run the following build procedure such that the new image can be pushed into GCR or any other Repo

```
gcloud config set project tranquility-base-images
docker build -t gcr.io/tranquility-base-images/tb-jenkins_latest:landingzone .
docker push gcr.io/tranquility-base-images/tb-jenkins_latest:landingzone

```

# Current list of supported plugins that currently reside in plugins.txt

```
jackson2-api
apache-httpcomponents-client-4-api
authentication-tokens
aws-credentials
aws-java-sdk
structs
trilead-api
command-launcher
credentials-binding:1.13
credentials-binding:1.16
credentials-binding:1.18
credentials-binding:1.23
bouncycastle-api
google-oauth-plugin
configuration-as-code
snakeyaml-api
handlebars
momentjs
pubsub-light
kubernetes
kubernetes-client-api
kubernetes-credentials
slack
amazon-ecs
amazon-ecr
ace-editor
ant
antisamy-markup-formatter
jsch
display-url-api
lockable-resources
structs
branch-api
cloudbees-folder
credentials
cvs
docker-plugin:1.2.0
docker-java-api:3.1.5
docker-commons
docker-workflow
durable-task
external-monitor-job
icon-shim
okhttp-api
config-file-provider
h2-api
oauth-credentials
htmlpublisher
hashicorp-vault-plugin
handy-uri-templates-2-api
git-client
git-server
git
github-api
github-branch-source
github
github-issues
github-oauth
github-organization-folder
github-pullrequest
github-pr-coverage-status
gitlab-merge-request-jenkins
gitlab-oauth
gitlab-plugin
google-login
google-compute-engine
google-storage-plugin
google-kubernetes-engine
google-play-android-publisher
google-cloudbuild
google-source-plugin
google-chat-notification
googleanalytics
google-git-notes-publisher
google-metadata-plugin
google-container-registry-auth
google-cloud-backup
googlecode
google-cloud-health-check
google-deployment-manager
google-chat-notification
#google-container-security-plugin
google-admin-sdk
gradle
javadoc
jquery-detached
jdk-tool
jira
junit
ldap
mapdb-api
mailer:1.32
#matrix-auth
matrix-project
maven-plugin
metrics
mercurial
pam-auth
pipeline-maven
pipeline-build-step
pipeline-graph-analysis
pipeline-input-step
pipeline-milestone-step
pipeline-model-api
pipeline-model-declarative-agent
pipeline-model-extensions
pipeline-model-definition
pipeline-githubnotify-step
pipeline-github-lib
pipeline-rest-api
pipeline-stage-step
pipeline-stage-tags-metadata
pipeline-stage-view
pipeline-utility-steps
plain-credentials
scm-api
script-security
ssh-credentials
ssh-slaves
subversion
translation
variant
windows-slaves
workflow-aggregator
workflow-api
workflow-basic-steps
workflow-cps-global-lib
workflow-cps
workflow-durable-task-step
workflow-job
workflow-multibranch
workflow-scm-step
workflow-step-api
workflow-support
favorite
token-macro
terraform
sse-gateway
cloudbees-bitbucket-branch-source
blueocean-display-url
blueocean-pipeline-scm-api
pipeline-stage-step
blueocean-dashboard
blueocean-pipeline-editor
blueocean-rest
blueocean-pipeline-api-impl
blueocean-git-pipeline
blueocean-commons
blueocean-jwt
blueocean-web
blueocean-rest-impl
jenkins-design-language
blueocean-config
blueocean-bitbucket-pipeline
blueocean-github-pipeline
blueocean-jira
blueocean-personalization
blueocean-core-js
blueocean-events
blueocean-i18n
blueocean
blueocean-autofavorite
gitlab-plugin
service-now
build-timeout
build-token-trigger
command-launcher
build-token-root
```